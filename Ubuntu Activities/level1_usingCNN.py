...
def threaded_image_handling(cv2_img):
	...
	model = load_model('trafficsignalmodel.h10')
	#print(model.summary())
	#test_image = cv2.imread(cwd+"\\test6.png")
	#img = np.reshape(test_image,[300,200,3])
	resized_image = cv2.resize(cv2_img, (640,480))
	test_image = np.expand_dims(resized_image, axis = 0)
	result = model.predict(test_image)
	global h
	h=result
	print("My pred:",result)
