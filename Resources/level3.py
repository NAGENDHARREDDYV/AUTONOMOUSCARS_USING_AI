...
def threaded_image_handling(cv2_img):
	...
	## you can change the get patches function according to you
	## or use your own model
	## Also, note in get patches function you will have to mention the correct paths of template/logo used for matching
	stop, right, left = get_patches(cv2_img)
	## you have 1 patch per sign coming in for each image
	## process if you want to take any and which to take
	## also if you say that there are 2 relevant patches then change front wall threshold and the angle at which lidar looks
	## As you can see on Line 254 LIDAR currently only measures distance at lidar angle 0
